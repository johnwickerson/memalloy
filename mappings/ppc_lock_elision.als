    ...
    all e : dom[X.scsl] {
      e not in ran[X.sb & X.scsl] => (
        ...
        some disj eLR, eLW, e' : X'.EV {
          ...
          eLW <: (X'.sb) in (X'.cd) & (isync[none->none,X']) 
                          // eLW is followed by a ctrl-isync
        }
      )
      e not in dom[X.sb & X.scsl] => (
        ...
        some disj e', eL : X'.EV {
          ...
          eL in L & X'.W // eL is a lock-related write 
          (X'.sb) :> eL in sync[none->none,X'] // eL is preceded by a sync
        }
      ) 
